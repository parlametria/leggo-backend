def get_destaque_query(destaque):
    q_destaque = ""
    if destaque == "true":
        q_destaque = (" AND id_leggo IN (SELECT DISTINCT(id_leggo) \
              FROM api_destaques \
...
def get_tipo_documento_query(tipo_documento):
    ...
    if tipo_documento:
        documentoArg = (
            tipo_documento.center(len(tipo_documento) + 4, "%")
            if tipo_documento
            else "%%"
        )
        q_tipo_documento = f" AND tipo_documento LIKE '{documentoArg}'"
...
def queryAutoriasAgregadasByTipoAcao(
    data_inicio, interesse, tema, destaque, tipo_acao_filtro, tipo_documento=None
):
    q_tema = get_tema_query(tema)
    q_destaque = get_destaque_query(destaque)
...
def queryAutoriasAgregadasByTipoAcaoEIdAutor(
    data_inicio,
    interesse,
    tema,
    destaque,
    ...
    core_query = queryAutoriasAgregadasByTipoAcao(
        data_inicio, interesse, tema, destaque, tipo_acao_filtro, tipo_documento
    )
    ...
    q = f"SELECT \
      ...
      FROM (SELECT \
      autoria_agg.*, \
      MAX(quantidade_autorias) OVER (ORDER BY quantidade_autorias DESC) \
          AS max_quantidade_autorias, \
      MIN(quantidade_autorias) OVER (ORDER BY quantidade_autorias ASC) \
          AS min_quantidade_autorias \
